VERSION 5.00
Object = "{831FDD16-0C5C-11D2-A9FC-0000F8754DA1}#2.0#0"; "Mscomctl.ocx"
Begin VB.Form frmDepartMent 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Department Information"
   ClientHeight    =   6990
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6075
   Icon            =   "frmDepartment.frx":0000
   LinkTopic       =   "Form2"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ScaleHeight     =   6990
   ScaleWidth      =   6075
   StartUpPosition =   2  'CenterScreen
   Begin MSComctlLib.ImageList ImageList1 
      Left            =   5400
      Top             =   0
      _ExtentX        =   1005
      _ExtentY        =   1005
      BackColor       =   -2147483643
      ImageWidth      =   13
      ImageHeight     =   13
      MaskColor       =   12632256
      _Version        =   393216
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         NumListImages   =   3
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDepartment.frx":08CA
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDepartment.frx":0D1C
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDepartment.frx":116E
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin VB.Frame Frame3 
      Caption         =   "List of Department Information"
      ForeColor       =   &H00000000&
      Height          =   4110
      Left            =   127
      TabIndex        =   14
      Top             =   2760
      Width           =   5820
      Begin MSComctlLib.ListView ListView1 
         Height          =   3735
         Left            =   120
         TabIndex        =   9
         Top             =   240
         Width           =   5580
         _ExtentX        =   9843
         _ExtentY        =   6588
         View            =   3
         LabelEdit       =   1
         Sorted          =   -1  'True
         LabelWrap       =   0   'False
         HideSelection   =   0   'False
         FullRowSelect   =   -1  'True
         GridLines       =   -1  'True
         _Version        =   393217
         Icons           =   "ImageList1"
         SmallIcons      =   "ImageList1"
         ColHdrIcons     =   "ImageList1"
         ForeColor       =   0
         BackColor       =   16777215
         BorderStyle     =   1
         Appearance      =   1
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Microsoft Sans Serif"
            Size            =   8.25
            Charset         =   0
            Weight          =   400
            Underline       =   0   'False
            Italic          =   0   'False
            Strikethrough   =   0   'False
         EndProperty
         NumItems        =   0
      End
   End
   Begin VB.Frame Frame2 
      Caption         =   "Navigation"
      ForeColor       =   &H00000000&
      Height          =   975
      Left            =   127
      TabIndex        =   13
      Top             =   1755
      Width           =   5820
      Begin VB.CommandButton cmdNew 
         Caption         =   "&Add"
         Enabled         =   0   'False
         Height          =   375
         Left            =   135
         MaskColor       =   &H8000000F&
         MousePointer    =   99  'Custom
         Style           =   1  'Graphical
         TabIndex        =   0
         Top             =   405
         Width           =   1095
      End
      Begin VB.CommandButton cmdClose 
         Caption         =   "C&lose"
         Height          =   375
         Left            =   4575
         MaskColor       =   &H8000000F&
         MousePointer    =   99  'Custom
         Style           =   1  'Graphical
         TabIndex        =   8
         Top             =   405
         Width           =   1095
      End
      Begin VB.CommandButton cmdCancel 
         Caption         =   "&Cancel"
         Height          =   375
         Left            =   3465
         MaskColor       =   &H8000000F&
         MousePointer    =   99  'Custom
         Style           =   1  'Graphical
         TabIndex        =   7
         Top             =   405
         Width           =   1095
      End
      Begin VB.CommandButton cmdUpdate 
         Caption         =   "&Update"
         Height          =   375
         Left            =   2355
         MaskColor       =   &H8000000F&
         MousePointer    =   99  'Custom
         Style           =   1  'Graphical
         TabIndex        =   6
         Top             =   405
         Width           =   1095
      End
      Begin VB.CommandButton cmdSave 
         Caption         =   "&Save"
         Height          =   375
         Left            =   1245
         MaskColor       =   &H8000000F&
         MousePointer    =   99  'Custom
         Style           =   1  'Graphical
         TabIndex        =   5
         Top             =   405
         Width           =   1095
      End
   End
   Begin VB.Frame Frame1 
      Caption         =   "Department Information"
      ForeColor       =   &H00000000&
      Height          =   1650
      Left            =   127
      TabIndex        =   10
      Top             =   120
      Width           =   5820
      Begin VB.CommandButton cmdFindDept 
         Height          =   300
         Left            =   2205
         Picture         =   "frmDepartment.frx":1A48
         Style           =   1  'Graphical
         TabIndex        =   18
         Tag             =   "PK"
         ToolTipText     =   "Search Screen"
         Top             =   360
         Width           =   315
      End
      Begin VB.CommandButton cmdFindGroup 
         Height          =   300
         Left            =   2205
         Picture         =   "frmDepartment.frx":1B42
         Style           =   1  'Graphical
         TabIndex        =   3
         Tag             =   "PK"
         ToolTipText     =   "Search Screen"
         Top             =   735
         Width           =   315
      End
      Begin VB.TextBox txtGroupID 
         BackColor       =   &H00FFFFFF&
         ForeColor       =   &H00000000&
         Height          =   285
         Left            =   1215
         MaxLength       =   25
         TabIndex        =   2
         ToolTipText     =   "Maximum 25 Characters"
         Top             =   735
         Width           =   975
      End
      Begin VB.TextBox txtGroupName 
         BackColor       =   &H00FFFFFF&
         ForeColor       =   &H00000000&
         Height          =   285
         Left            =   3570
         Locked          =   -1  'True
         MaxLength       =   25
         TabIndex        =   15
         ToolTipText     =   "Maximum 25 Characters"
         Top             =   735
         Width           =   2055
      End
      Begin VB.TextBox txtDeptId 
         BackColor       =   &H00FFFFFF&
         ForeColor       =   &H00000000&
         Height          =   285
         Left            =   1215
         MaxLength       =   3
         TabIndex        =   1
         ToolTipText     =   "Maximum 3 Characters"
         Top             =   360
         Width           =   975
      End
      Begin VB.TextBox txtDeptName 
         BackColor       =   &H00FFFFFF&
         ForeColor       =   &H00000000&
         Height          =   285
         Left            =   1215
         MaxLength       =   25
         TabIndex        =   4
         ToolTipText     =   "Maximum 25 Characters"
         Top             =   1110
         Width           =   4410
      End
      Begin VB.Label Label2 
         Alignment       =   1  'Right Justify
         AutoSize        =   -1  'True
         BackColor       =   &H80000018&
         BackStyle       =   0  'Transparent
         Caption         =   "Group ID:"
         ForeColor       =   &H00000000&
         Height          =   195
         Left            =   480
         TabIndex        =   17
         Top             =   735
         Width           =   690
      End
      Begin VB.Label Label1 
         Alignment       =   1  'Right Justify
         AutoSize        =   -1  'True
         BackColor       =   &H80000018&
         BackStyle       =   0  'Transparent
         Caption         =   "Group Name:"
         ForeColor       =   &H00000000&
         Height          =   195
         Left            =   2550
         TabIndex        =   16
         Top             =   735
         Width           =   990
      End
      Begin VB.Label lblAreaId 
         Alignment       =   1  'Right Justify
         AutoSize        =   -1  'True
         BackColor       =   &H80000018&
         BackStyle       =   0  'Transparent
         Caption         =   "Dept. ID:"
         ForeColor       =   &H00000000&
         Height          =   195
         Left            =   525
         TabIndex        =   12
         Top             =   360
         Width           =   645
      End
      Begin VB.Label lblAreaName 
         Alignment       =   1  'Right Justify
         AutoSize        =   -1  'True
         BackColor       =   &H80000018&
         BackStyle       =   0  'Transparent
         Caption         =   "Dept. Name:"
         ForeColor       =   &H00000000&
         Height          =   195
         Left            =   270
         TabIndex        =   11
         Top             =   1110
         Width           =   900
      End
   End
End
Attribute VB_Name = "frmDepartMent"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Private Sub cmdCancel_Click()
On Error GoTo ErrHandle:
Dim Control As Object
For Each Control In Me
    If TypeOf Control Is TextBox Then
        Control.Text = ""
    End If
Next
Call prc_DisAllow
txtDeptId.Enabled = True
txtDeptId.BackColor = &HFFFFFF
Exit Sub
ErrHandle:
    Me.MousePointer = vbDefault
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdClose_Click()
On Error GoTo ErrHandle:
    Unload Me
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdFindDept_Click()
On Error GoTo ErrHandle:
    frmSearchDept.Show vbModal
If strfindcode <> "" Then
    txtDeptId = strfindcode
    txtDeptID_Validate True
    strfindcode = ""
Else
    '--Nothing
End If
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdFindGroup_Click()
On Error GoTo ErrHandle:
    frmSearchGroup.Show vbModal
If strfindcode <> "" Then
    txtGroupID = strfindcode
    txtGroupID_Validate True
    strfindcode = ""
Else
    '--Nothing
End If
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdNew_Click()
On Error GoTo ErrHandle:
Dim Control As Object
For Each Control In Me
    If TypeOf Control Is TextBox Then
        Control.Text = ""
    End If
Next
txtDeptId.Enabled = False
txtDeptId.BackColor = &H80000018
Call prc_Allow
txtDeptName.SetFocus
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdSave_Click()
On Error GoTo ErrHandle:
If Trim(txtDeptName) = "" Then
    MsgBox "Department Name can't be blank! Please Enter Department Name.", vbExclamation, "Remark Message"
    txtDeptName.SetFocus
    Exit Sub
ElseIf Trim(txtGroupID) = "" Then
    MsgBox "Group ID can't be blank! Please Enter Group ID.", vbExclamation, "Remark Message"
    txtGroupID.SetFocus
    Exit Sub
ElseIf Trim(txtGroupName) = "" Then
    MsgBox "Group Name can't be blank! Please Enter Group Name.", vbExclamation, "Remark Message"
    txtGroupName.SetFocus
    Exit Sub
End If
'--Name Already Exist
Dim rsName As New Recordset
rsName.Open "Select vDeptName From tblDepartment Where vDeptName='" & txtDeptName & "'", CnnMain, adOpenForwardOnly, adLockReadOnly
If Not rsName.EOF Then
    MsgBox "Department name already exist! Department Name should not be duplicate!", vbExclamation, "Remark Message"
    txtDeptName.SetFocus
 Exit Sub
End If
Set rsName = Nothing
'------sp_Add_CBtblZone
Me.MousePointer = vbHourglass
CnnMain.BeginTrans
CnnMain.Execute "Exec Sp_Add_tblDept '" & Trim(txtDeptName) & "'," & Trim(txtGroupID) & ""
CnnMain.CommitTrans
Dim rsDeptID As New Recordset
rsDeptID.Open "Select siDeptID From tblDepartment Order By siDeptID Desc", CnnMain, adOpenForwardOnly, adLockReadOnly
If Not rsDeptID.EOF Then
    txtDeptId = rsDeptID!siDeptID & ""
End If
Set rsDeptID = Nothing
Call prc_DisAllow
Me.MousePointer = vbDefault
MsgBox "Record successfully saved to the database.", vbInformation, "Remark Message"
Call cmdDataSearch_Click
txtDeptId.Enabled = True
txtDeptId.BackColor = &HFFFFFF
txtDeptName.SelStart = 0
txtDeptName.SelLength = Len(txtDeptName)
txtDeptName.SetFocus
Exit Sub
ErrHandle:
    Me.MousePointer = vbDefault
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub ListView1_ColumnClick(ByVal ColumnHeader As MSComctlLib.ColumnHeader)
    ListView1.ColumnHeaders.Item(ListView1.SortKey + 1).Icon = 0
    Call Module3.lvSortByColumn(ListView1, ColumnHeader)
    ListView1.ColumnHeaders.Item(ListView1.SortKey + 1).Icon = ListView1.SortOrder + 1
End Sub

Private Sub ListView1_DblClick()
On Error GoTo ErrHandle:
    Call prc_DisAllow
    txtDeptId = ListView1.SelectedItem.Text
    txtDeptID_Validate True
    Exit Sub
ErrHandle:
        MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
        Err.Clear
End Sub

Private Sub ListView1_KeyPress(KeyAscii As Integer)
On Error GoTo ErrHandle:
If KeyAscii = 13 Then
    Call prc_DisAllow
    txtDeptId = ListView1.SelectedItem.Text
    txtDeptID_Validate True
End If
Exit Sub
ErrHandle:
        MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
        Err.Clear
End Sub

Private Sub txtDeptID_KeyPress(KeyAscii As Integer)
    If KeyAscii = 8 Then Exit Sub
    If KeyAscii < 12 Or KeyAscii > 14 And KeyAscii < 48 Or KeyAscii > 57 Then KeyAscii = 0
    If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then txtDeptID_Validate False
End Sub

Private Sub txtDeptID_Validate(Cancel As Boolean)
On Error GoTo ErrHandle:
If txtDeptId = "" Then
    cmdNew_Click
    Exit Sub
End If
Dim rsDept As New ADODB.Recordset
rsDept.Open "Select vDeptName,siGroupID From tblDepartment Where siDeptId='" & Trim(txtDeptId) & "'", CnnMain, adOpenForwardOnly, adLockReadOnly
If Not rsDept.EOF Then
    Dim rsGroup As New Recordset
    With rsDept
        txtDeptName = !vDeptName & ""
        txtGroupID = !siGroupID & ""
        rsGroup.Open "Select vGroupName From tblGroup Where siGroupID=" & rsDept!siGroupID & "", CnnMain, adOpenForwardOnly, adLockReadOnly
        txtGroupName = rsGroup!vGroupName & ""
        rsGroup.Close
        rsDept.MoveNext
    End With
    Set rsGroup = Nothing
    Set rsDept = Nothing
    txtDeptName.SelStart = 0
    txtDeptName.SelLength = Len(txtDeptName)
    txtDeptName.SetFocus
    prc_DisAllow
Else
    MsgBox "Invalid Department ID!", vbExclamation, "Validation Message"
    Cancel = True
    txtDeptId.SelStart = 0
    txtDeptId.SelLength = Len(txtDeptId)
    txtDeptName = ""
    txtGroupID = ""
    txtGroupName = ""
End If
Set rsDept = Nothing
If txtDeptId.Enabled = False Then
    txtDeptId.Enabled = True
    txtDeptId.BackColor = &HFFFFFF
End If
Exit Sub
ErrHandle:
    Me.MousePointer = vbDefault
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub cmdUpdate_Click()
On Error GoTo ErrHandle:
If Trim(txtDeptName) = "" Then
    MsgBox "Department Name can't be blank! Please Enter Department Name.", vbExclamation, "Remark Message"
    txtDeptName.SetFocus
    Exit Sub
ElseIf Trim(txtGroupID) = "" Then
    MsgBox "Group ID can't be blank! Please Enter Group ID.", vbExclamation, "Remark Message"
    txtGroupID.SetFocus
    Exit Sub
ElseIf Trim(txtGroupName) = "" Then
    MsgBox "Group Name can't be blank! Please Enter Group Name.", vbExclamation, "Remark Message"
    txtGroupName.SetFocus
    Exit Sub
End If
CnnMain.BeginTrans
CnnMain.Execute "Exec Sp_Up_tblDept " & Trim(txtDeptId) & ",'" & Trim(txtDeptName) & "'," & Trim(txtGroupID) & ""
CnnMain.CommitTrans
Me.MousePointer = vbDefault
MsgBox "Record successfully update to the database.", vbInformation, "Update Message"
Call cmdDataSearch_Click
txtDeptName.SelStart = 0
txtDeptName.SelLength = Len(txtDeptName)
txtDeptName.SetFocus
Exit Sub
ErrHandle:
    Me.MousePointer = vbDefault
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub
Private Sub Form_Load()
On Error GoTo ErrHandle:
Me.Move (Screen.Width - Me.Width) / 2, (frmMain.ScaleHeight - Me.Height) / 2
Call prc_DisAllow
ListView1.ColumnHeaders.Add , , "Dept. ID", 1400
ListView1.ColumnHeaders.Add , , "Dept. Name", 2000
ListView1.ColumnHeaders.Add , , "Group Name", 2100
'Call SetHeaderFontStyle(ListView1) '---For Bold Column Header
ListView1.ColumnHeaders.Item(ListView1.SortKey + 1).Icon = ListView1.SortOrder + 1
Call cmdDataSearch_Click
Me.MousePointer = vbDefault
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub
Public Sub prc_DisAllow()
Dim obj As Object
  For Each obj In Me
        If TypeOf obj Is CommandButton Then
                If obj.Name = "cmdSave" Or obj.Name = "cmdCancel" Then
                obj.Enabled = False
        Else
                obj.Enabled = True
                End If
        End If
    Next
End Sub
Public Sub prc_Allow()
Dim obj As Object
    For Each obj In Me
        If TypeOf obj Is CommandButton Then
            If obj.Name = "cmdSave" Or obj.Name = "cmdCancel" Or obj.Name = "cmdFindDept" Or obj.Name = "cmdFindGroup" Then
                obj.Enabled = True
            Else
                obj.Enabled = False
            End If
        End If
    Next
End Sub
Public Sub clear_all_textboxes()
    txtDeptId = ""
    txtDeptName = ""
    txtGroupID = ""
    txtGroupName = ""
End Sub
Private Sub txtGroupID_KeyPress(KeyAscii As Integer)
    If KeyAscii = 8 Then Exit Sub
    If KeyAscii < 12 Or KeyAscii > 14 And KeyAscii < 48 Or KeyAscii > 57 Then KeyAscii = 0
    If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then txtGroupID_Validate True
End Sub

Private Sub txtGroupID_Validate(Cancel As Boolean)
On Error GoTo ErrHandle:
If txtGroupID = "" Then
    txtGroupName = ""
    Exit Sub
End If
Dim rsGroup As New Recordset
rsGroup.Open "Select vGroupName From tblGroup Where siGroupID='" & Trim(txtGroupID) & "'", CnnMain, adOpenForwardOnly, adLockReadOnly
If Not rsGroup.EOF Then
    txtGroupName = rsGroup!vGroupName & ""
    txtDeptName.SetFocus
Else
    MsgBox "Invalid Group ID! Please enter valid Group ID!", vbExclamation, "Remark Message"
    Cancel = True
    txtGroupID.SelStart = 0
    txtGroupID.SelLength = Len(txtGroupID)
    txtGroupID.SetFocus
    txtGroupName = ""
    Exit Sub
End If
Set rsGroup = Nothing
Exit Sub
ErrHandle:
    Me.MousePointer = vbDefault
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

Private Sub txtGroupName_KeyPress(KeyAscii As Integer)
    If KeyAscii = 39 Then KeyAscii = 0
End Sub

Private Sub cmdDataSearch_Click()
On Error GoTo ErrHandle:
Dim rsAdd As New Recordset
rsAdd.Open "Select * From tblDepartment Order By vDeptName Asc", CnnMain, adOpenForwardOnly, adLockReadOnly
Dim itmX As ListItem
ListView1.ListItems.Clear
If Not rsAdd.EOF Then
    Dim rsGroup As New Recordset
    Do While Not rsAdd.EOF
        Set itmX = ListView1.ListItems.Add(, , rsAdd!siDeptID, , 3)
        itmX.SubItems(1) = rsAdd!vDeptName & ""
        rsGroup.Open "Select vGroupName From tblGroup Where siGroupID=" & rsAdd!siGroupID & "", CnnMain, adOpenForwardOnly, adLockReadOnly
        itmX.SubItems(2) = rsGroup!vGroupName & ""
        rsGroup.Close
        rsAdd.MoveNext
    Loop
    Set rsGroup = Nothing
End If
Set rsAdd = Nothing
Exit Sub
ErrHandle:
    MsgBox "ID : " & Err.Number & Chr(10) & "Description : " & Err.Description, vbExclamation, "Call Database Administrator"
    Err.Clear
End Sub

